#include <stdint.h>
#include <stdio.h>
#include "radix_ntt_n1277.h"
#include "../poly.h"
#include "../inverse.h"

int32_t zetas_n1277[N_N1277 / 5] = {
...
int32_t zetas_inv_n1277[N_N1277 / 5] = {
...
int32_t montgomery_reduce_n1277(int64_t a)
{
    int32_t t;
...
void ntt_big_n1277(int32_t a[N_N1277])
{
        ...
        for (start = 0; start < N_N1277; start = j + len)
        {
            ...
            if (zeta == NUMBER_CSNNH)
            {
                for (j = start; j < start + len; ++j)
                {
                    t = a[j + len];
                    a[j + len] = (a[j] + t);
                    a[j] = pseudomersenne_reduce_single_n1277(a[j] - t);
                }
            }
            else
            {
                for (j = start; j < start + len; ++j)
                {
                    t = montgomery_reduce_n1277((int64_t)zeta * a[j + len]);
                    a[j + len] = (a[j] - t);
                    a[j] = (a[j] + t);
                }
            }
...
void ntt_small_n1277(int32_t a[N_N1277])
{
    unsigned int len, start, j, k;
    int32_t zeta, t;
...
void invntt_toNUMBER_CSNLKMH(int32_t a[N_N1277])
{
    ...
    for (len = 5; len <= N_N1277 / 4; len <<= 1)
    {
        if (len == (5 << 3))
        {
            for (j = 0; j < N_N1277; j++)
                a[j] = pseudomersenne_reduce_single_n1277(a[j]);
        }

        for (start = 0; start < N_N1277; start = j + len)
        {
            zeta = zetas_inv_n1277[k++];
            ...
            else
            {
                for (j = start; j < start + len; ++j)
                {
                    ...
                    a[j] = (t + a[j + len]);
                    a[j + len] = (t - a[j + len]);
                    a[j + len] = montgomery_reduce_n1277((int64_t)zeta * a[j + len]);
                }
            }
        }
        // if(len == (5 << 5)){
    ...
    for (start = 0; start < N_N1277; start = j + len)
    {
        ...
        {
            t = a[j];
            a[j] = pseudomersenne_reduce_single_n1277(t + a[j + len]);
            a[j + len] = pseudomersenne_reduce_single_n1277(a[j + len] - t);
        }
    }
}

#define KARA(a, b, x, y, d) ((montgomery_reduce_n1277((int64_t)(a[x] + a[y]) * (b[x] + b[y])) - d[x] - d[y]))

void basemul_n1277(int32_t c[5], const int32_t a[5], const int32_t b[5], const int32_t zeta)
{
    int i;
    int32_t d[5];

    for (i = 0; i < 5; i++)
        d[i] = montgomery_reduce_n1277((int64_t)a[i] * b[i]);

    c[0] = (d[0] + montgomery_reduce_n1277((int64_t)zeta * (KARA(a, b, 1, 4, d) + KARA(a, b, 2, 3, d))));
    c[1] = (KARA(a, b, 0, 1, d) + montgomery_reduce_n1277((int64_t)zeta * (KARA(a, b, 2, 4, d) + d[3])));
    c[2] = (KARA(a, b, 0, 2, d) + d[1] + montgomery_reduce_n1277((int64_t)zeta * (KARA(a, b, 3, 4, d))));
    c[3] = (KARA(a, b, 0, 3, d) + KARA(a, b, 1, 2, d) + montgomery_reduce_n1277((int64_t)zeta * (d[4])));
    c[4] = (KARA(a, b, 0, 4, d) + KARA(a, b, 1, 3, d) + d[2]);
}

void poly_ntt_big_n1277(nttpoly_n1277 *a)
{
    ntt_big_n1277(a->coeffs);
}

void poly_ntt_small_n1277(nttpoly_n1277 *a)
{
    ntt_small_n1277(a->coeffs);
}

void poly_invntt_n1277(nttpoly_n1277 *a)
{
    invntt_toNUMBER_CSNLKMH(a->coeffs);
}
...
void poly_basemul_n1277(nttpoly_n1277 *c, const nttpoly_n1277 *a, const nttpoly_n1277 *b)
{
    int i;
    int32_t zeta;
    for (i = 0; i < N_N1277 / 10; i++)
    {
        zeta = zetas_n1277[N_N1277 / 10 + i];
        basemul_n1277(c->coeffs + 10 * i, a->coeffs + 10 * i, b->coeffs + 10 * i, zeta);
        basemul_n1277(c->coeffs + 10 * i + 5, a->coeffs + 10 * i + 5, b->coeffs + 10 * i + 5, -zeta);
    }
}
...
{
    b->coeffs[0] = fq_freeze(montgomery_reduce_n1277((int64_t)FACTOR_DLXHNDNSHX * (a->coeffs[0] + a->coeffs[FPTRU_N])));
    for (int i = 1; i <= FPTRU_N - 2; i++)
        b->coeffs[i] = fq_freeze(montgomery_reduce_n1277((int64_t)FACTOR_DLXHNDNSHX * (a->coeffs[i] + a->coeffs[i + FPTRU_N - 1] + a->coeffs[i + FPTRU_N])));
    b->coeffs[FPTRU_N - 1] = fq_freeze(montgomery_reduce_n1277((int64_t)FACTOR_DLXHNDNSHX * (a->coeffs[FPTRU_N - 1] + a->coeffs[2 * FPTRU_N - 2])));
}

void poly_extract_n1277_q2(poly *b, const nttpoly_n1277 *a)
{
    b->coeffs[0] = (montgomery_reduce_n1277((int64_t)FACTOR_DLXHNDNSHX * (a->coeffs[0] + a->coeffs[FPTRU_N]))) & (FPTRU_Q2 - 1);
    for (int i = 1; i <= FPTRU_N - 2; i++)
        b->coeffs[i] = (montgomery_reduce_n1277((int64_t)FACTOR_DLXHNDNSHX * (a->coeffs[i] + a->coeffs[i + FPTRU_N - 1] + a->coeffs[i + FPTRU_N]))) & (FPTRU_Q2 - 1);
    b->coeffs[FPTRU_N - 1] = (montgomery_reduce_n1277((int64_t)FACTOR_DLXHNDNSHX * (a->coeffs[FPTRU_N - 1] + a->coeffs[2 * FPTRU_N - 2]))) & (FPTRU_Q2 - 1);
}

void poly_radix_ntt_n1277_q1(poly *c, const poly *a, const poly *b)
{
    nttpoly_n1277 ntta, nttb, nttc;

    poly_extend_n1277(&ntta, a);
